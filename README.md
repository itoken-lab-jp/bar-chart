...
Power BI のカスタムビジュアルの棒グラフです。標準の棒グラフと同じように使えるようにしたうえで、標準では届かないグラフ表現を足していきます。いまは縦棒・横棒の集合・積み上げ・100% 積み上げ、折れ線との複合（縦棒）、リボン（縦棒）に対応しています。
...
- 凡例と複数の値（集合縦棒）。「凡例」にフィールドを入れるか、「値」に2つ以上入れると系列に分かれます。凡例の位置（12通り）・文字・タイトル、系列ごとの色・透過性・罫線、系列間のスペース、データラベルの系列ごとの表示と色を設定できます。
- 積み上げ縦棒と 100% 積み上げ縦棒、合計ラベル（正と負の分割）。積み上げのツールヒントには合計、100% 積み上げのツールヒントには割合を出します。
- 折れ線との複合（折れ線および集合縦棒・折れ線および積み上げ縦棒）。「折れ線の値」に入れたメジャーを折れ線で描き、棒と尺度が違えば右の第 2 Y 軸に出します。第 2 Y 軸（範囲・値・タイトル）、線（色・幅・線のスタイル）、マーカーを設定できます。
- 横棒（集合横棒・積み上げ横棒・100% 積み上げ横棒）。書式設定のカードは標準と同じく、値の軸が「X 軸」、カテゴリの軸が「Y 軸」になります。
- リボン。カテゴリごとに値の大きい順に上から積み、同じ系列を隣のカテゴリと帯でつなぎます。帯にマウスを乗せると、前後の値・変化・順位を出します。帯の色・透過性・罫線・リボンと列の間のスペースを設定できます。
...
- グラフの種類（集合・積み上げ・100% 積み上げ・リボン）と向き（縦棒・横棒）は、書式設定の「列」で切り替えます（標準は視覚化ペインで別のビジュアルを選びます）。
- 横棒では折れ線を描きません（標準の複合も縦棒だけです）。横棒のグリッド線は、値の線に「横」、カテゴリの区切りに「縦」の設定を使います。
- リボンの書式は系列ごとには変えられません（すべての帯に同じ設定）。横棒のリボンは帯を描かず、値の大きい順に積むだけです。
- データラベルと合計ラベルの表示単位は、Y 軸の表示単位に従います（ラベル側では選べません）。100% 積み上げのデータラベルは、値ごとに単位を付けます（1,250億 など）。
- 並べ替えは、書式設定の「列」にある「値順で並べ替え」「順序を逆にする」で行います。系列が複数のときは、カテゴリの合計の大きい順です。
...
- 小さな複数
- 折れ線のデータラベル・網掛け領域・系列ラベル、マーカーの形
- 凡例の「表示順を反転」、列の「重複」、データラベルの系列ごとの設定のうち表示と色以外
